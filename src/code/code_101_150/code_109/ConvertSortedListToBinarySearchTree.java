...
public class ConvertSortedListToBinarySearchTree {
    ...
    private TreeNode doBuildBST(int leftIndex, int rightIndex) {
        if (leftIndex >= rightIndex) {
            return null;
        }
        int midIndex = (leftIndex + rightIndex) >>> 1;
        TreeNode leftNode = doBuildBST(leftIndex, midIndex);
        TreeNode res = new TreeNode(this.head.val);
        res.left = leftNode;
        this.head = this.head.next;
        res.right = doBuildBST(midIndex + 1, rightIndex);
        return res;
    }
}
